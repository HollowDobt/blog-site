...
<Modal bind:open={deleteView}>
	<div id="delete" class="flex flex-col items-center justify-center gap-5">
		...
		<time>{t("comment.time")}: {Time.toString(comment.timestamp)}</time>
			...
			{#if comment.name !== null}
				...
				<dt class="flex flex-col gap-0.5 min-w-0">
					<p class="flex items-center gap-1">
						...
						<span>·</span>
						<time class="text-xs">{Time.toString(comment.updated ?? comment.timestamp).replace("-", " ")}</time>
					</p>
			...
			{:else}
				...
				<dt class="flex flex-col gap-0.5 min-w-0">
					<p class="flex items-center gap-1">
						...
						{/if}
						<span>·</span>
						<time class="text-xs">{Time.toString(comment.timestamp).replace("-", " ")}</time>
					</p>
				</dt>
			{/if}
		</div>
